    ...
    with Live(progress, console=console, refresh_per_second=4):
        try:
            ...
            while iteration < max_iterations:
                ...
                # Normalize to lowercase for consistent comparison with WorkflowState enum
                workflow_state = status.get("workflow_state", "initializing").lower()
            ...
            try:
                # Validate research plan exists before accessing
                if director.research_plan is None:
                    logger.warning("Research plan not initialized")
                else:
